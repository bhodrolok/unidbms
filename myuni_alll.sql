...
SELECT pr.Name, p.name
	...
	WHERE ProfId = pr.Prof_id and pr.Program_id = p.Program_id;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `view_student_app` (IN `StudentID` INT)  BEGIN
...
INSERT INTO `student` (`Student_id`, `Username`, `Admin_id`, `Year_enrolled`, `Major`, `Name`, `GPA`, `App_status`) VALUES
(1, 'student1', 1, 2016, 'CS', 'John Doe', 4.3, 'Finalized'),
(2, 'student2', 2, 2020, 'History', 'Jane Doeson', 3.99, 'Submitted');
...
INSERT INTO `user_login` (`Username`, `Password`, `usertype`) VALUES
('admin1', 'admin1', 'admin'),
('admin2', 'admin2', 'admin'),
('prof1', 'prof1', 'prof'),
...
--
ALTER TABLE `admin`
  ADD CONSTRAINT `Username_admin` FOREIGN KEY (`Username`) REFERENCES `user_login` (`Username`);
...
ALTER TABLE `prof`
  ...
  ADD CONSTRAINT `Username_prof` FOREIGN KEY (`Username`) REFERENCES `user_login` (`Username`);
...
ALTER TABLE `student`
  ...
  ADD CONSTRAINT `Username_student` FOREIGN KEY (`Username`) REFERENCES `user_login` (`Username`);
